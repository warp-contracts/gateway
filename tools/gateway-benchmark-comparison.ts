...
async function gatewayComparisonBenchmark() {
  ...
  const timeSpentArweave = await loadFromAweaveGateway(
    contractId,
    fromBlockHeight,
    toBlockHeight
  );

  const timeSpentRedstone = await loadFromRedstoneGateway(
    contractId,
    fromBlockHeight,
    toBlockHeight
  );
...
async function loadFromRedstoneGateway(
  ...
  for (let i = 0; i < 3; i++) {
    await loader.load(
      ...
      parseInt(fromBlockHeight as string),
...
async function loadFromAweaveGateway(
  contractId: string,
  fromBlockHeight?: string,
  toBlockHeight?: string
) {
