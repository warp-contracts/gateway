...
export async function interactionsSortKeyRoute_v3(ctx: Router.RouterContext) {
  const { dbSource, logger } = ctx;

  const { contractId, confirmationStatus, limit, from, to, totalCount, source, fromSdk } = ctx.query;
