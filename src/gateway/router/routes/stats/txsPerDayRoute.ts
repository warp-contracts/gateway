...
export async function txsPerDayRoute(ctx: Router.RouterContext) {
  ...
  const contracts: any = await dbSource.raw(
    `
        WITH contracts_per_day AS (SELECT date(to_timestamp((block_timestamp)::integer)) as date,
                                          contract_id as interaction
                                   FROM contracts
                                   WHERE testnet ${testnet ? ' IS NOT NULL' : ' IS NULL'})
        SELECT date, count(*) as per_day
        FROM contracts_per_day
        GROUP BY date
        ORDER BY date ASC;
    `
  );
  const interactions: any = await dbSource.raw(
    `
        WITH transactions_per_day AS (SELECT date(to_timestamp((block_timestamp)::integer)) as date,
