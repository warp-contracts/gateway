...
export async function gcpAliveRoute(ctx: Router.RouterContext) {
  ...
  ctx.body = {
    ...
    ...(arBlockHeight && { ar_block_height: arBlockHeight }),
  };
}

async function dbAccessible(ctx: Router.RouterContext, arBlockHeight: number | null) {
  const { dbSource } = ctx;
  ...
  if (dbSource.healthCheckEnabled()) {
    try {
      ...
      return {
        ...
        ...(arBlockHeight && { l1_interaction_height_diff: arBlockHeight - result.rows[0].l1_max_bh }),
      };
    } catch (e: any) {
      return {
        up: 0,
