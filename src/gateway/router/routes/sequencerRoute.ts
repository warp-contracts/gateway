...
async function doGenerateSequence(ctx: Router.RouterContext, trx: Knex.Transaction): Promise<string> {
  ...
  if (acquireMutexResult.lastSortKey == null
  ...
  const sortKey = await createSortKey(arweave, jwk, acquireMutexResult.blockHash, millis, transaction.id, acquireMutexResult.blockHeight);
